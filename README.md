# Akashgowda K S

## C++ / Embedded Software Developer

Developer @ **Sapiens** | Life & Pension | CS - Delivery CoE

Ex- **Capgemini Engineering** | Embedded C, IoT & Systems Programming

---

## 👋 About Me

I am a **C++ Developer** currently working at **Sapiens** in the Life & Pension CS - Delivery CoE, contributing to the development and maintenance of enterprise-scale insurance products. My work emphasizes **performance optimization, reliability, and clean system design** in large C++ codebases, following structured SDLC and engineering best practices.

Previously, I worked in the **Embedded & IoT** domain at Capgemini Engineering, where I developed firmware and communication layers for **ARM-based ABB devices**, implemented **MQTT**, and integrated edge systems with **Microsoft Azure Cloud** for Oil & Gas applications.

I hold a **Bachelor of Engineering in Electronics and Communication Engineering** and have a strong foundation in **systems programming, embedded development, and backend data handling**.

---

## 🧰 Tech Stack & Tooling

### Languages & Systems

![C](https://img.shields.io/badge/C-00599C?style=flat\&logo=c\&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat\&logo=c%2B%2B\&logoColor=white)
![Embedded C](https://img.shields.io/badge/Embedded-C-informational)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat\&logo=linux\&logoColor=black)

### Databases

![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat\&logo=mysql\&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat\&logo=mysql\&logoColor=white)

### DevOps & Workflow

![Git](https://img.shields.io/badge/Git-F05032?style=flat\&logo=git\&logoColor=white)
![Bitbucket](https://img.shields.io/badge/Bitbucket-0052CC?style=flat\&logo=bitbucket\&logoColor=white)
![JIRA](https://img.shields.io/badge/JIRA-0052CC?style=flat\&logo=jira\&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI%2FCD-Bitbucket_Pipelines-blue)

### Cloud & IoT

![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat\&logo=microsoftazure\&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-660066?style=flat)
![ARM](https://img.shields.io/badge/ARM-0091BD?style=flat\&logo=arm\&logoColor=white)
![RTOS](https://img.shields.io/badge/RTOS-Embedded-informational)

---

## 📌 Featured Projects (Pinned Repositories)

> *Representative project ideas aligned with my professional experience.*

* **Life & Pension, CS- Delivery CoE, Core and Cust Modules (C++)**
  Performance-focused C++ components for enterprise insurance systems, emphasizing scalability, memory efficiency, and maintainability.

* **MQTT Client for Embedded Devices**
  Lightweight MQTT implementation for ARM-based embedded boards with secure cloud integration.

* **Embedded Linux Utilities**
  System-level tools and services written in C/C++ for Linux-based embedded platforms.

* **SQL Performance & Analytics**
  Advanced SQL queries, window functions, and performance-tuning examples.

---

## 💼 Professional Experience

### Sapiens — C++ Developer

*October 2022 – Present | Bangalore, India*

* Maintain and enhance core and cust Life & Pension insurance products (IFGL project).
* Optimize C++ modules for performance, stability, and scalability.
* Collaborate with cross-functional teams across development, QA, and product.
* Contribute to improved client satisfaction through reliable product delivery.

### Capgemini Engineering

**Professional 1 – Engineer**
*April 2022 – September 2022 | Gurugram, India*

* Implemented MQTT protocol on ABB embedded boards for IoT communication.
* Integrated ARM-based systems with RTOS and Embedded Linux.
* Enabled secure data transmission to Azure Cloud for Oil & Gas solutions.

**Associate II – Software Engineer**
*June 2020 – March 2022 | Gurugram, India*

* Worked as an Embedded Software Developer using Embedded C.
* Contributed to firmware-level development and system integration.

### Aricent — Software Trainee

*June 2019 – May 2020 | Gurugram, India*

### Emertxe Information Technologies — Graduate Trainee (Intern)

*September 2018 – May 2019 | Bengaluru, India*

---

## 🎓 Education

**APS College of Engineering**
Bachelor of Engineering (B.E.) — Electronics and Communication Engineering
*2014 – 2018*

---

## 📜 Certifications

* Bitbucket Pipelines for CI/CD – Level Up: SQL
* Advanced SQL – Window Functions
* SQL Server: Security, Encryption, and Masking
* Microsoft Azure Fundamentals
* MATLAB R2018a – Latest Features and Capabilities

---

## 📄 Publications

* **Performance Analysis of MIMO-OFDM System using Pilot-Based Channel Estimation Technique**

---

## 📊 GitHub Stats

![Akashgowda's GitHub stats](https://github-readme-stats.vercel.app/api?username=akashgowda-ks\&show_icons=true\&theme=default)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=akashgowda-ks\&layout=compact)

---

## 📫 Contact

* 📍 Bengaluru, Karnataka, India
* 📧 Email: [Akashgowda.KS@sapiens.com](mailto:Akashgowda.KS@sapiens.com)
* 🔗 LinkedIn: [https://www.linkedin.com/in/akashgowdaks](https://www.linkedin.com/in/akashgowdaks)
* 🔗 Microsoft Learn: [https://learn.microsoft.com/en-us/users/akashgowdaks-5754/](https://learn.microsoft.com/en-us/users/akashgowdaks-5754/)


---

*I use GitHub to explore modern C++, embedded systems, performance engineering, and scalable software design.*
